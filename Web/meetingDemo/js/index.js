layui.use(['form', 'layer', 'laytpl', 'element'], function () {
    ...
    class _loginM {
        constructor() {
            ...
            setTimeout(() => {
                ...
                if (paramsObj.meetid !== undefined) {
                    ...
                    // 会议产品特有参数，SDK用户无需关注
                    if (paramsObj.pw) { // 房间密码
                        $('#g_meet_pwd').val(paramsObj.pw);
                    }
                    ...
                    if (paramsObj.nickname === undefined) {
                        window.layer.prompt({
                            ...
                            MeetingDemo.Login.userID = `${value.toString()}_${Math.floor(Math.random() * 8999) + 1000}`;
                            $('#userInputName').val(value.toString());
                        ...
                        this.userID = paramsObj.userid ? paramsObj.userid : `${this.nickname}_${Math.floor(Math.random() * 8999) + 1000}`;
        ...
        registerCallback() {
            ...
            CRVideo_LoginSuccess.callback = (UID, cookie) => {
                ...
                if (cookie == 'urlEnter' || cookie == 'loginAndEnter') { // 链接入会 或者 非自动登录状态下点击进入房间按钮
                    MeetingDemo.RoomMgr.enterMeetingFun(cookie);
                } else if (cookie == 'loginAndCreate') { // 非自动登录状态下点击创建房间按钮
                    MeetingDemo.RoomMgr.onClickCreateBtn();
                } else { // 自动登录，登录成功
                    !window.isWebMeetingApp && MeetingDemo.tipLayer(`登录成功：${UID}`);
                    !!window.isWebMeetingApp && MeetingDemo.removeLoading(); // 隐藏加载层
                }
            }
            // SDK接口：回调 登录失败
        ...
        initSDK(enterType) {
            ...
            CRVideo_SetSDKParams({
                // isSDKConsole: false,
                isUploadLog: this.server == 'crlab.cloudroom.com:543' ? false : true,
                MSProtocol: this.protocol, // 媒体流传输协议 TCP UDP 详见文档
                isCallSer: !!window.isWebMeetingApp ? false : true, // 是否启用队列呼叫服务
            });
        ...
        loginSystem(cookie) {
            ...
            // 不连接房间外消息服务（内部webApp特殊处理，SDK开发者无需关注）
            !!window.isWebMeetingApp && CRVideo_SetSDKParams({
                isCallSer: false
            });
            ...
            if (nickname.length > 10 || UID.length > 10) return MeetingDemo.tipLayer('昵称太长');
    ...
    class _roomMgr {
        ...
        enterMeetingFun(cookie) {
            if (!!window.isWebMeetingApp) { // 会议产品特有参数，SDK用户无需关注
                CRMeetApp_EnterMeeting(this.meetID, $('#g_meet_pwd').val(), MeetingDemo.Login.userID, MeetingDemo.Login.nickname, 'meetApp');
            } else {
                // SDK接口：进入房间
        ...
        enterMeetingRslt(sdkErr, cookie) {
            ...
            if (sdkErr == 0) {
                ...
                MeetingDemo.Sync.getCurPage(); // 查询当前功能区
                setTimeout(() => {
                ...
                if (![1, 9, 205, 800, 801, 802, 805, 806].includes(sdkErr)) {
    ...
    class _memberM {
        ...
        createUserIcon(UID, videoID, UIID) {
            let parsedID = UID.replace(':', '');
            parsedID = parsedID.replace(/\./g, '');
            const htmlStr =
                `<div class="deviceStatus" data-userid="${parsedID}" data-videoid="${videoID}" data-UIID="${UIID}">
        ...
        onClickRename(dom) {
            ...
            window.layer.prompt({
                ...
                // SDK接口：修改成员昵称
                CRVideo_SetNickName(UID, value).then(res => {
    ...
    class _videoM {
        ...
        initData() {
            ...
            this.vbimgurl = '';
            $('.bgimg-choose').hide();
            $('.choose-list li:first-child').addClass('active').siblings().removeClass('active');
        }
        registerCallback() {
            // SDK接口：通知 成员的摄像头设备变化
        ...
        refreshUserVideoWall(page = 0) {
            ...
            // 不允许翻到空白页
            if (page * this.pageVideoNum >= allVideoArr.length) return MeetingDemo.Video.lastPage();

            // 找出本页this.pageVideoNum个视频
            const pageVideos = [];
            let isChanged = false;
            for (let i = (0 + page * this.pageVideoNum), j = 0;
                (i < allVideoArr.length && j < this.pageVideoNum); i++) {
        ...
        updateMemberVideoUI(member) {
            ...
            member.allVideos.forEach(videoItem => {
                if (!videoItem.videoUI) {
                    ...
                    const videoStyle = { // video标签的自定义样式
                        objectFit: 'cover' //object-fit属性，cover表示裁剪显示
                    }

                    // SDK接口：创建成员视频UI组件
                    let videoUI = window.CRVideo_CreatVideoObj({ // 可以传一个属性对象，都是video标签支持的属性
                        poster: posterImg,
                        // style: videoStyle
                    });
                ...
                if (member.userID == UID) {
                    member.allVideos.forEach(videoItem => {
                        if (videoItem.videoUI && videoItem.videoUI.id() == uiid) {
                            ...
                            const picName = `${UID}-${videoItem.videoUI.getVideoID()}-${Date.year}-${Date.month}-${Date.day}-${Date.hour}-${Date.minute}-${Date.second}.png`;
        ...
        addScreenCam() {
            ...
            // 获取桌面媒体流
            navigator.mediaDevices.getDisplayMedia(screenCfg).then(stream => {
        ...
        removeScreenCam() {
            ...
            if (allVideo.length <= 1 && this.screenCamID) { // 本身没有其他摄像头，只有一个桌面摄像头
                window.CRVideo_CloseVideo(MeetingDemo.Login.userID); // SDK接口：关闭摄像头
            } else {
                // 如果当前正在现在底默认摄像头就是桌面摄像头，应先切换到其它摄像头
        ...
        // 点击虚拟背景设置按钮
        onClickVBGBtn() {
            const baseUrl = MeetingDemo.Login.server.includes('localhost') || MeetingDemo.Login.server.includes('127.0.0.1') ? 'sdk.cloudroom.com' : MeetingDemo.Login.server;
            $('#vbImg1').data('url', `https://${baseUrl}/web/resource/imgs/vb1.jpg`);
            $('#vbImg2').data('url', `https://${baseUrl}/web/resource/imgs/vb2.jpg`);
            $('#vbImg3').data('url', `https://${baseUrl}/web/resource/imgs/vb3.jpg`);
            $('#vbImg1 .img-box img').attr('src', `https://${baseUrl}/web/resource/imgs/vb1.jpg`);
            $('#vbImg2 .img-box img').attr('src', `https://${baseUrl}/web/resource/imgs/vb2.jpg`);
            $('#vbImg3 .img-box img').attr('src', `https://${baseUrl}/web/resource/imgs/vb3.jpg`);
            $('.bgimg-choose').show();
        }
        // 点击选择虚拟背景图片
        chooseVBImg(dom) {
        ...
        onClickVBConfirmBtn() {
            $('.bgimg-choose').hide();
            // 打开虚拟背景
            if (!!$('.choose-list li.active').data('url')) {
                if ($('#mutiCam')[0].checked == true) MeetingDemo.tipLayer('多摄像头模式下虚拟背景将不可用')
                const url = $('.choose-list li.active').data('url');
                const jsonCfg = {
                    type: 2, // 0:不启用  1:绿幕模式  2:人像模式
                    LANData: true, // 是否使用内网服务器数据而非公网数据（无法访问公网的情况）
                    imgUrl: url // 背景图
                }
                console.crlog(`[MeetingDemo] 设置虚拟背景`);
                window.CRVideo_SetVirtualBackground(jsonCfg).then(res => {
                    $('#mutiCam').attr('disabled', true);
                    if (this.vbimgurl == '') {
                        this.vbimgurl = url;
                        window.CRVideo_CloseVideo(MeetingDemo.Login.userID);
                        setTimeout(() => {
                            window.CRVideo_OpenVideo(MeetingDemo.Login.userID);
                        }, 1000);
                    }
                }).catch(err => {
                    console.log(err);
                    MeetingDemo.tipLayer(`设置虚拟背景失败：${err.errDesc}`)
                })

                // 关闭虚拟背景
            } else {
                const jsonCfg = {
                    type: 0, // 0:不启用  1:绿幕模式  2:人像模式
                }
                console.crlog(`[MeetingDemo] 关闭虚拟背景`);
                window.CRVideo_SetVirtualBackground(jsonCfg).then(res => {
        ...
        // 打开摄像头失败
        openVideoFailed(err) {
            MeetingDemo.alertLayer(`打开摄像头失败：${err} </br>您的摄像头设备可能被占用或未授权访问，请检查后重新开关摄像头再试！`);
        }
    }
        ...
        onClickScreenShareBtn(dom) {
            if (MeetingDemo.Media.isStarting || MeetingDemo.Media.isPlaying) { // 当前没有在播放影音
                ...
                if ($(".page_share_box").css("display") == "none") {
                    $(".page_share_box").show();
                    $(".page_meet_video").hide();
                    !window.isWebMeetingApp && MeetingDemo.Sync.switchToPage(6);
                } else {
                    $(".page_share_box").hide();
                    $(".page_meet_video").show();
                    !window.isWebMeetingApp && MeetingDemo.Sync.switchToPage(8);
                }
            } else { // 当前没有在屏幕共享或播放影音
        ...
        onClickMediaShareBtn() {
            ...
            if (MeetingDemo.Screen.isSharing) {
                ...
                if ($(".page_share_box").css("display") == "none") {
                    $(".page_share_box").show();
                    $(".page_meet_video").hide();
                    !window.isWebMeetingApp && MeetingDemo.Sync.switchToPage(7);
                } else {
                    $(".page_share_box").hide();
                    $(".page_meet_video").show();
                    !window.isWebMeetingApp && MeetingDemo.Sync.switchToPage(8);
    ...
    class _inviteM {
        ...
        // 发送邀请
        sendInvite(UID) {
            CRVideo_Invite(UID, JSON.stringify({
                "meeting": {
    ...
    class _chatM {
        ...
        registerCallback() {
            ...
            CRVideo_NotifyMeetingCustomMsg.callback = (fromUserID, stringMsg) => {
                ...
                if (jsonMsg.CmdType && jsonMsg.CmdType == 'IM' && !window.isWebMeetingApp) this.recivedChatMsg(fromUserID, jsonMsg.IMMsg);
            }
            // SDK接口：通知  收到IM消息（已废弃，兼容旧版）
            CRVideo_NotifyIMMsg.callback = (userID, text, sendTime) => {
                !!window.isWebMeetingApp && this.recivedChatMsg(userID, unescape(text));
            }
        }
        // 点击工具栏聊天按钮
        onClickChatBtn() {
    ...
    class _SyncClass {
        ...
        initData() {
            // this.lastMainPage = 8; // 上一次的主功能区
            this.curMainPage = 0; // 当前主功能区
            this.curSubPage = '0.0'; // 当前子页面
        }
        registerCallback() {
            // SDK接口：通知 当前功能区有变化
            window.CRVideo_NotifySwitchToPage.callback = (mainPage, subPage) => {
                console.crlog(`[MeetingDemo] 通知功能区切换，主功能区：${mainPage}，子功能区：${subPage}`);
                if (mainPage == 4) {
                    MeetingDemo.Board.changCurBoardPage(subPage);
                }
                if (this.curMainPage == mainPage) return;
                this.curMainPage = mainPage;
                this.curSubPage = subPage;
                this.switchToPage(mainPage, subPage, true);
            }
        }
        // 获取当前功能区
        getCurPage() {
            this.curMainPage = window.CRVideo_GetCurrentMainPage(); // SDK接口：获取当前主功能区
            this.curSubPage = window.CRVideo_GetCurrentSubPage(); // SDK接口：获取当前子功能区
            console.crlog(`[MeetingDemo] 获取到当前主功能区：${this.curMainPage}，子功能区：${this.curSubPage}`);
            this.switchToPage(this.curMainPage, this.curSubPage, true);
        }

        // 切换功能区
        switchToPage(mainPage, subPage, isNotify) {
            if (!MeetingDemo.RoomMgr.isMeeting) return;
            !isNotify && console.crlog(`[MeetingDemo] 切换功能区：${mainPage}, ${subPage}`);
            // 如果只是通知切换，就不要调接口了
            !isNotify && window.CRVideo_SwitchToPage(mainPage, subPage == undefined ? '0.0' : subPage); // SDK接口：切换功能区

            switch (mainPage) {
                case 4: // 白板
                    MeetingDemo.Board.changCurBoardPage(subPage);
                    MeetingDemo.tipLayer(`当前主功能区：白板`);
                    if (isNotify) break;
                    $('.page_board_box').show();
                    $('.page_meet_video').hide();
                    $('.page_share_box').hide();
    ...
    class _BoardClass {
        ...
        initData() {
            ...
            this.curBoardUI = null;
            this.penStyle = {
                strokeStyle: '#0000FF', // 画笔颜色
                lineWidth: 2, // 画笔线宽
                cursor: 'url("./image/pc/meeting_pen1_.ico"), auto', // 画笔样式
            }
        }
        loadEvent() {
            // 线宽选择列表
            $('.mark-tool-btn.linewidth').hover(e => {
                $('.item-list.line-width-list').show();
            }, e => {
                $('.item-list.line-width-list').hide();
            })
            $('.item-list.line-width-list').hover(e => {
                $('.item-list.line-width-list').show();
            }, e => {
                $('.item-list.line-width-list').hide();
            })
            // 颜色选择列表
            $('.mark-tool-btn.color').hover(e => {
                $('.item-list.line-color-list').show();
            }, e => {
                $('.item-list.line-color-list').hide();
            })
            $('.item-list.line-color-list').hover(e => {
                $('.item-list.line-color-list').show();
            }, e => {
                $('.item-list.line-color-list').hide();
            })
            // 线宽列表项hover
            $('.item-list.line-width-list .active i').css('backgroundColor', this.penStyle.strokeStyle).parent().siblings().children('i').css('backgroundColor', '#49505c');
            $('.item-list.line-width-list li').hover(e => {
                $(e.currentTarget).children('i').css('backgroundColor', this.penStyle.strokeStyle);
            }, e => {
                if (!$(e.currentTarget).hasClass('active')) $(e.currentTarget).children('i').css('backgroundColor', '#49505c');
            })
        }
        registerCallback() {
            // SDK接口：通知 初始化白板列表
            window.CRVideo_NotifyInitBoards.callback = (CRVideo_BoardObjs) => {
                this.boardList = CRVideo_BoardObjs;
                this.boardList.forEach(item => {
                    const boardUI = this.createBoardUI();
                    boardUI.setCurBoardPage(item.boardID, item.curPage);
                    item.boardUI = boardUI;
                })
            }
            // SDK接口：通知 创建了新白板
            window.CRVideo_NotifyCreateBoard.callback = (CRVideo_BoardObj, UID) => {
                const boardUI = this.createBoardUI();
                boardUI.setCurBoardPage(CRVideo_BoardObj.boardID, CRVideo_BoardObj.curPage);
                CRVideo_BoardObj.boardUI = boardUI;
                this.boardList.push(CRVideo_BoardObj);
                this.changCurBoardPage(CRVideo_BoardObj.boardID);
            }
            // SDK接口：通知 关闭了白板
            window.CRVideo_NotifyCloseBoard.callback = (boardID, UID) => {
                const closedIndex = this.boardList.findIndex(item => item.boardID == boardID);
                closedIndex > -1 && this.boardList.splice(closedIndex, 1);
                if (closedIndex == 0) {
                    $('.board-main').html('当前没有白板');
                } else {
                    UID == MeetingDemo.Login.userID && MeetingDemo.Sync.switchToPage(4, this.boardList[this.boardList.length - 1].boardID);
                }
            }
            // SDK接口：通知 白板进行翻页
            window.CRVideo_NotifyBoardCurPageNo.callback = (boardID, curPage, pagePos1, pagePos2, UID) => {
                this.boardList.forEach(item => {
                    if (item.boardID == boardID) {
                        item.curPage = curPage;
                        item.pagePos1 = pagePos1;
                        item.pagePos2 = pagePos2;
                        item.boardUI.setCurBoardPage(item.boardID, item, curPage);
                        this.changCurBoardPage(item.boardID);
                    }
                })
            }
        ...
        // 创建白板，并设置白板的一些属性
        createBoardUI() {
            const boardUI = window.CRVideo_CreateBoardUIObj(); // SDK接口：创建白板
            boardUI.setTitleStyle({ // SDK接口：白板显示组件 —— 设置白板标题样式
                opacity: 0.3,
                marginTop: '0.05rem',
            })
            boardUI.setBoardStyle({ // SDK接口：白板显示组件 —— 设置白板样式
                backgroundColor: '#FFF'
            })
            boardUI.setPen(this.penStyle); // SDK接口：白板显示组件 —— 设置画笔样式
            boardUI.isScaledContents(false); // SDK接口：白板显示组件 —— 设置（查询）是否允许缩放
            return boardUI;
        }
        // 切换当前的白板和页码
        changCurBoardPage(boardID) {
            console.log(`切换当前白板：${boardID}`);
            this.boardList.forEach(boardItem => {
                if (boardItem.boardID == boardID) {
                    this.curBoardPage = {
                        boardID: boardID,
                        curPage: boardItem.curPage,
                        pagePos1: boardItem.pagePos1,
                        pagePos2: boardItem.pagePos2,
                        pageCount: boardItem.pageCount,
                    };
                    console.log(`切换白板页：${boardID}, ${boardItem.curPage}`);
                    boardItem.boardUI && boardItem.boardUI.setCurBoardPage(boardItem.boardID, boardItem.curPage);
                    $('#pageCur').text(boardItem.curPage + 1);
                    $('#pageTotal').text(boardItem.pageCount);
                    boardItem.boardUI && (this.curBoardUI = boardItem.boardUI);
                    boardItem.boardUI && $('.board-main').html(boardItem.boardUI.handler());
                }
            })
        }
        // 点击白板按钮
        onClickSwitchToBoardBtn() {
            console.log(`切换白板功能区`);
            if (MeetingDemo.Sync.curMainPage !== 4) {
                if (this.boardList.length == 0) {
                    this.newBoard(undefined, undefined, undefined, 1);
                } else {
                    MeetingDemo.Sync.switchToPage(4, this.boardList[this.boardList.length - 1].boardID);
                }
            } else {
                MeetingDemo.Sync.switchToPage(8);
            }
        ...
        // 新建白板按钮
        createNewBoard() {
            let title, width, height, count;
            MeetingDemo.promptLayer(2, '请输入图片白板标题', '', (value) => {
                title = value;
                MeetingDemo.promptLayer(2, '请输入白板宽度', '800', value => {
                    width = value;
                    MeetingDemo.promptLayer(2, '请输入白板高度', '600', value => {
                        height = value;
                        MeetingDemo.promptLayer(2, '请输入白板页数', '1', value => {
                            count = value;
                            this.newBoard(title, width, height, count);
                        })
                    })
                })
            })
        ...
        // 白板翻页：上一页
        lastPage() {
            if (this.curBoardPage.curPage !== 0) {
                // SDK接口：白板翻页
                window.CRVideo_SetBoardCurPageNo(this.curBoardPage.boardID, this.curBoardPage.curPage - 1, this.curBoardPage.pagePos1, this.curBoardPage.pagePos2);
            } else {
                MeetingDemo.tipLayer('已经是第一页了');
            }
        }
        // 白板翻页：下一页
        nextPage() {
            if (this.curBoardPage.curPage < this.curBoardPage.pageCount - 1) {
                // SDK接口：白板翻页
                window.CRVideo_SetBoardCurPageNo(this.curBoardPage.boardID, this.curBoardPage.curPage + 1, this.curBoardPage.pagePos1, this.curBoardPage.pagePos2);
            } else {
                MeetingDemo.tipLayer('已经是最后一页了');
            }
        }
        // 新建白板
        newBoard(name = 'H5_MeetingDemo_Board', width = 1120, height = 630, pageCount = 3) {
            // SDK接口：创建白板
            CRVideo_Creatboard(name, width, height, pageCount).then(res => {
                console.log(`创建白板成功`);
                MeetingDemo.Sync.switchToPage(4, res.boardID);
            }).catch(err => {
                console.log(`白板创建失败`);
            })
        }
        ...
        closeDoc() {
            // SDK接口：关闭白板
            window.CRVideo_Closeboard(this.curBoardPage.boardID).then(res => {
                console.log('关闭白板成功');
            }).catch(err => {
                console.log('关闭白板失败');
            })
        ...
        // 清空图元
        clear() {
            if (this.curBoardUI) this.curBoardUI.clear();
        }
        // 撤销
        undo() {
            if (this.curBoardUI) this.curBoardUI.undo();
        }
        // 恢复
        redo() {
            if (this.curBoardUI) this.curBoardUI.redo();
        }
        // 缩放
        toggleScale() {
            if (this.curBoardUI) this.curBoardUI.isScaledContents(!this.curBoardUI.isScaledContents());
        }
        // 导入背景
        importBgImg() {
            ...
            inputEle.onchange = (e) => {
                ...
                const imgFile = inputEle.files[0];
                const url = URL.createObjectURL(imgFile);
                const img = new Image();
                img.src = url;
                img.onload = () => {
                    this.curBoardUI.importImg(img);
                }
            }
        ...
        // 点击选择线宽
        onChooseLineWidth(lw, dom) {
            this.penStyle.lineWidth = lw;
            $(dom).addClass('active').siblings().removeClass('active');
            this.loadEvent();
            this.curBoardUI.setPen({
                lineWidth: lw
            })
        }
        // 点击选择颜色
        onChooseColor(color, dom) {
            this.penStyle.strokeStyle = color;
            $(dom).addClass('active').siblings().removeClass('active');
            this.loadEvent();
            this.curBoardUI.setPen({
                strokeStyle: color
    ...
    MeetingDemo.Board = new _BoardClass();
    ...
    class _CloudMixerClass {
        ...
        createUnaflowMixer() {
            ...
            const { year, month, day } = MeetingDemo.getDate();
            const svrPath = `/${year}-${month}-${day}/`;
            ...
            if (isCreateMp4File) {
                ...
                videoFileCfg.svrPath = svrPath;
                videoFileCfg.svrFileNameSuffix = '.mp4';
            ...
            if (isCreateMp3File) {
                ...
                const audioFileCfg = {};
                audioFileCfg.svrPath = svrPath;
                audioFileCfg.svrFileNameSuffix = '.mp3';
