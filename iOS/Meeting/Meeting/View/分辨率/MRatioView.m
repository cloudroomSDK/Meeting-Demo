...
@interface MRatioView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
- (IBAction)clickBtnForMRatioView:(UIButton *)sender;

@end

@implementation MRatioView
...
- (void)showAnimation {
    // 选中当前选项
    NSNumber *value = [_dataMap objectForKey:_curRatio];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    contentLabel.text = [self.dataSource objectAtIndex:row];
    contentLabel.font = [UIFont systemFontOfSize:18];
    return contentLabel;
}

- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    _curRatio = [self.dataSource objectAtIndex:row];
}
...
- (IBAction)clickBtnForMRatioView:(UIButton *)sender {
    if (_response) {
        _response(self, sender, _curRatio);
    }
